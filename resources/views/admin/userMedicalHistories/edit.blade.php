...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userMedicalHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-medical-histories.update", [$userMedicalHistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="disease_name">{{ trans('cruds.userMedicalHistory.fields.disease_name') }}</label>
                <input class="form-control {{ $errors->has('disease_name') ? 'is-invalid' : '' }}" type="text" name="disease_name" id="disease_name" value="{{ old('disease_name', $userMedicalHistory->disease_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $userMedicalHistory->description) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_history') ? 'is-invalid' : '' }}" name="user_history_id" id="user_history_id">
                    @foreach($user_histories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_history_id') ? old('user_history_id') : $userMedicalHistory->user_history->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('care_histories') ? 'is-invalid' : '' }}" name="care_histories[]" id="care_histories" multiple required>
                    @foreach($care_histories as $id => $care_history)
                        <option value="{{ $id }}" {{ (in_array($id, old('care_histories', [])) || $userMedicalHistory->care_histories->contains($id)) ? 'selected' : '' }}>{{ $care_history }}</option>
                    @endforeach
                </select>
