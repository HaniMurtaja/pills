    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reminders.update", [$reminder->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('times') ? 'is-invalid' : '' }}" type="text" name="times" id="times" value="{{ old('times', $reminder->times) }}" required>
                @if($errors->has('times'))
                    <div class="invalid-feedback">
                        {{ $errors->first('times') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('days_of_week') ? 'is-invalid' : '' }}" type="text" name="days_of_week" id="days_of_week" value="{{ old('days_of_week', $reminder->days_of_week) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('snooze') ? 'is-invalid' : '' }}" type="text" name="snooze" id="snooze" value="{{ old('snooze', $reminder->snooze) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_reminder') ? 'is-invalid' : '' }}" name="user_reminder_id" id="user_reminder_id">
                    @foreach($user_reminders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_reminder_id') ? old('user_reminder_id') : $reminder->user_reminder->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('care_reminders') ? 'is-invalid' : '' }}" name="care_reminders[]" id="care_reminders" multiple required>
                    @foreach($care_reminders as $id => $care_reminder)
                        <option value="{{ $id }}" {{ (in_array($id, old('care_reminders', [])) || $reminder->care_reminders->contains($id)) ? 'selected' : '' }}>{{ $care_reminder }}</option>
                    @endforeach
                </select>
