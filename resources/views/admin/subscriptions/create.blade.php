    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.subscriptions.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" type="number" name="payment_method" id="payment_method" value="{{ old('payment_method', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('subsription_date') ? 'is-invalid' : '' }}" type="text" name="subsription_date" id="subsription_date" value="{{ old('subsription_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_subs') ? 'is-invalid' : '' }}" name="user_subs_id" id="user_subs_id">
                    @foreach($user_subs as $id => $entry)
                        <option value="{{ $id }}" {{ old('user_subs_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
