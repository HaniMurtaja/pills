...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.userDoc.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-docs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('care_docs') ? 'is-invalid' : '' }}" name="care_docs[]" id="care_docs" multiple required>
                    @foreach($care_docs as $id => $care_doc)
                        <option value="{{ $id }}" {{ in_array($id, old('care_docs', [])) ? 'selected' : '' }}>{{ $care_doc }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_doc') ? 'is-invalid' : '' }}" name="user_doc_id" id="user_doc_id">
                    @foreach($user_docs as $id => $entry)
                        <option value="{{ $id }}" {{ old('user_doc_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
