    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('service_price') ? 'is-invalid' : '' }}" type="text" name="service_price" id="service_price" value="{{ old('service_price', $order->service_price) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" type="number" name="payment_method" id="payment_method" value="{{ old('payment_method', $order->payment_method) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('service_description') ? 'is-invalid' : '' }}" name="service_description" id="service_description">{{ old('service_description', $order->service_description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('arriving_date') ? 'is-invalid' : '' }}" type="text" name="arriving_date" id="arriving_date" value="{{ old('arriving_date', $order->arriving_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_orders') ? 'is-invalid' : '' }}" name="user_orders[]" id="user_orders" multiple>
                    @foreach($user_orders as $id => $user_order)
                        <option value="{{ $id }}" {{ (in_array($id, old('user_orders', [])) || $order->user_orders->contains($id)) ? 'selected' : '' }}>{{ $user_order }}</option>
                    @endforeach
                </select>
