    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.medicines.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('med_generic_name') ? 'is-invalid' : '' }}" type="text" name="med_generic_name" id="med_generic_name" value="{{ old('med_generic_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('med_scientific_name') ? 'is-invalid' : '' }}" type="text" name="med_scientific_name" id="med_scientific_name" value="{{ old('med_scientific_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('med_quantity') ? 'is-invalid' : '' }}" type="text" name="med_quantity" id="med_quantity" value="{{ old('med_quantity', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('med_expire_date') ? 'is-invalid' : '' }}" type="text" name="med_expire_date" id="med_expire_date" value="{{ old('med_expire_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_med') ? 'is-invalid' : '' }}" name="user_med_id" id="user_med_id">
                    @foreach($user_meds as $id => $entry)
                        <option value="{{ $id }}" {{ old('user_med_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('care_meds') ? 'is-invalid' : '' }}" name="care_meds[]" id="care_meds" multiple required>
                    @foreach($care_meds as $id => $care_med)
                        <option value="{{ $id }}" {{ in_array($id, old('care_meds', [])) ? 'selected' : '' }}>{{ $care_med }}</option>
                    @endforeach
                </select>
